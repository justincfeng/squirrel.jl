...
#-----------------------------------------------------------------------
"""
    nullenforcerf( V0::RealVec , X::RealVec , gfunc::Function , p ) 
...
"""
function nullenforcerf( V0::RealVec , X::RealVec , gfunc::Function , p )  
    tpfl=typeof(V0[1])
    nx = length(V0)
...
#-----------------------------------------------------------------------
"""
    nullenforcerp( V0::RealVec , X::RealVec , gfunc::Function , p ) 
...
"""
function nullenforcerp( V0::RealVec , X::RealVec , gfunc::Function , p )
    tpfl=typeof(V0[1])
    nx = length(V0)
...
#-----------------------------------------------------------------------
"""
    HamGeo( Z::RealVec , gfunc::Function , p )
...
"""
function HamGeo( Z::RealVec , gfunc::Function , p )
    tpfl=typeof(Z[1])
...
#-----------------------------------------------------------------------
"""
    ZdotGeo( Z::RealVec , gfunc::Function , p )

The `ZdotGeo` function computes the gradient of the Hamiltonian and
applies the symplectic operator to the gradient.

"""
function ZdotGeo( Z::RealVec , gfunc::Function , p )
    tpfl=typeof(Z[1])

    VH = ForwardDiff.gradient(z->HamGeo(z,gfunc,p),Z)           
    return Jsympl( VH )
end     #---------------------------------------------------------------

#-----------------------------------------------------------------------
"""
    ZF!( dZ , Z , gfunc::Function , p )

The function `ZF` converts the function `ZdotGeo` into a function with
mutable arguments.

"""
function ZF!( dZ , Z , gfunc::Function , p )
    copyto!(dZ, ZdotGeo( Z, gfunc , p ))
end     #---------------------------------------------------------------

#-----------------------------------------------------------------------
"""
    solveZ( Z0::RealVec , gfunc::Function, p , δ1::Real , δ2::Real ,
...
"""
function solveZ( Z0::RealVec , gfunc::Function , p , δ1::Real , δ2::Real , 
                integrator=AutoVern7(Rodas5()) , δt=0 ; 
                cb=CallbackSet() )
    tpfl=typeof(Z0[1])
    ...
    Zo = vcat(Z0[1:4],gfunc(Z0[1:4],p)*Z0[5:8]) # Lowering index of p 

    F = (dZ,Z,p,t)->ZF!( dZ , Z , gfunc ,p )

    if δt == 0
        slv = solve( ODEProblem(F,Zo,tspan,p),integrator,reltol=δ1,
